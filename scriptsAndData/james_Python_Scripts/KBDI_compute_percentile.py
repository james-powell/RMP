...
Author  : James Powell
Date    : 7/2/20
Purpose : This script will compute the daily climatolgical percentile for a 
...
csvFile = "%s/dataFiles/KBDI_data/%s_KBDI_daily_values.csv" % (filePath, mystnin )
...
if fileExist == True:
    ...
    string_time = np.genfromtxt(csvFile, delimiter = ",", skip_header = 2, usecols = 0, dtype=str  )
    KBDI        = np.genfromtxt(csvFile, delimiter = ",", skip_header = 2, usecols = 1, dtype=float)

    #print(string_time)
    #print('\n', KBDI)

    # Get the string dates in the format MMDD
    long_time  = pd.to_datetime(string_time, format= '%m/%d/%Y')
    ...
    for day in days:
        ...
        for perc in percentileList:
            ...
            mypctout = np.nanpercentile(KBDI[find],perc)
    ...
    print( 'Moving on to save the file: ', '%s/dataFiles/percentiles/%s_percentiles_KBDI.csv' % (filePath, mystnin))
    outhandle = open('%s/dataFiles/percentiles/%s_percentiles_KBDI.csv' % (filePath, mystnin),'w')
    outheader = 'MODY,'		
