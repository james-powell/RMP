...
for station in stations:
    print('__________________________STARTING WITH STATION: %s__________________________\n' % (station))

    fireStartTime = datetime(2020,5,12,13,39)
    fireName      = 'Saddle'

    endDate = datetime.today()
    endDate = datetime(2020,5,13)
    ...
    url = 'https://api.synopticdata.com/v2/stations/timeseries?&token=%s&units=english,speed|mph,temp|f&obtimezone=utc&vars=relative_humidity,wind_speed,wind_gust,peak_wind_speed,air_temp&output=json&stid=%s&recent=1800' % (token, station )
    ...
    try:
        ...
        print('ERROR: no data for peakWindSpeed' )
        peakWindSpeed = 0
        maxWind = windGust
    ...
    if station[0:3] == 'PC0' or station[0:2] == 'P0' or station == 'UTLPC':
        ## tHIS SECTION GETS THE DATA FOR THE 95TH PERCENTILE
    ...
    for y, varName, fileName, thres in zip(varList, varListNames, varFileNames, threshold):        
        ...
        if varName == 'Relative Humidity':
            ...
            ax.scatter(time_dt[dataUpper], y[dataUpper],  s =10, label = '%s Data' % (varName))
        ...
        # Plot info for FFWI
            ...
            #print('len(runTime[dataLower]): ', len(runTime[dataLower]))
            #print('len(y[dataLower]): ', len(y[dataLower]))
            # This plots the y with a different color if the values exceed the threshold
        ...
        ax.axvline(fireStartTime, color='black', linestyle='dashed', lw=1.25, alpha=0.7, label = 'Approx. Fire Start Time')
        ax.set_ylabel( '%s' %(varName))
        ax.set_xlabel( 'Month-Day Hour (MDT)')
        ax.set_title('%s %s Extreme Events the Day of The %s Fire' %(station, varName, fireName) )   

        # set the time period
        ax.set_xlim( xmin=startTimeGraph, xmax=endDate)

        lgd = ax.legend(loc='upper center', bbox_to_anchor=(.5,1.28), ncol=3, shadow = True)#, fancybox=True, shadow=True)
        ...
        fig.savefig('%s/pngFiles/extremePercentilePics/__TRIAL__%s_%s_Extreme_%s_24Hrs.png' % (filePath, station, endDate.year, fileName),  bbox_extra_artists=(lgd,), bbox_inches='tight')
        print('Saving the figure: ', '%s/pngFiles/extremePercentilePics/%s_%s_Extreme_%s_24Hrs.png' % (filePath, station, endDate.year, fileName))
        plt.close('all')

    print('____________________________DONE WITH STATION: %s____________________________\n' % (station))
