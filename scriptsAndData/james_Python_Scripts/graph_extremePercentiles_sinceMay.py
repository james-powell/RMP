...
for station in stations:
    ...
    # If it is outside the current fire season then do not
    if 5 > endDate.month < 11:
        pass
    else:
        ...
        for var, varFileName in zip(percentileList,varFileNames):
            ...
            var['MODY'] = organizeDates(endDate, var)
        ...
        for y, varName, fileName, thres in zip(varList, varListNames, varFileNames, threshold):
            try:
                ...
                fig.savefig('%s/pngFiles/extremePercentilePics/%s_Extreme_%s_sinceMay.png' % (filePath, station, fileName),  bbox_extra_artists=(lgd,), bbox_inches='tight')
                print('Saving the figure: ', '%s/pngFiles/extremePercentilePics/%s_Extreme_%s_sinceMay.png' % (filePath, station, fileName))
                plt.close(fig)
