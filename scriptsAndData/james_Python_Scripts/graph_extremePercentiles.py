...
for station in stations:
    ...
    try:
        ...
        if station[0:3] == 'PC0' or station[0:2] == 'P0':
            ...
            print(' statement for \'PC\' was tripped I am running the percentiles from the value: ', compareDir[station])
        ...
        for y, varName, fileName, thres in zip(varList, varListNames, varFileNames, threshold):
            ...
            while date1 < len(period):
                ...
                try:
                    ...
                    fig.savefig('%s/pngFiles/extremePercentilePics/%s_%s_Extreme_%s_%s.png' % (filePath, station, endDate.year, fileName, periodFileName[date1]),  bbox_extra_artists=(lgd,), bbox_inches='tight')
                    print('Saving the figure: ', '%s/pngFiles/extremePercentilePics/%s_%s_Extreme_%s_%s.png' % (filePath, station, endDate.year, fileName, periodFileName[date1]))
                    plt.close(fig)
                except:
                    print('\n-------------------------------------------------------------------------------------------------')
